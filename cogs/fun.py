import os, sys, discord, random, asyncio, math, youtube_dl, functools, itertools
...
class Song:
    ...
    def create_embed(self):
        ...
        embed.add_field(name='URL', value='[Open in YouTube]({0.source.url})'.format(self))
...
class VoiceState:
    ...
    async def audio_player_task(self):
        while True:
            ...
            if self.loop == False:
                ...
                # reasons.
                try:
                    async with timeout(655360):  # No limit ig
    ...
    @commands.command(name='abandon', aliases=['disconnect'])
    ...
    async def _volume(self, ctx: commands.Context, *, volume: int):
        ...
        if not ctx.voice_state.is_playing:
            return await ctx.send('⚠ Unable to change the Volume. Play some music first.')
    ...
    @commands.command(name='nowplaying', aliases=['current', 'playing', 'np'])
    ...
    @commands.command(name='next', aliases=['s'])
    async def _skip(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.is_playing:
            return await ctx.send('🛑 Queue is empty, nothing to skip.')
        ...
        if voter == ctx.voice_state.current.requester:
            ...
            if total_votes >= 3:
                await ctx.send('⏭ Votes are now counted. Skipping on it.')
                ctx.voice_state.skip()
            else:
                await ctx.send('⏭ Skipping by **{}/3**.'.format(total_votes))

        else:
            await ctx.send('🛑 Pardon but you have already voted to skip this song.')
    ...
    async def _shuffle(self, ctx: commands.Context):
        ...
        await ctx.send('🔀 Current playlist is being shuffled.')
    ...
    async def _remove(self, ctx: commands.Context, index: int):
        ...
        await ctx.send('✅ Removed one entry.')
    ...
    async def _loop(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.is_playing:
            return await ctx.send('🛑 Unable to loop, queue is empty.')

        # Inverse boolean value to loop and unloop.
        ctx.voice_state.loop = not ctx.voice_state.loop
        await ctx.send('I am on it.')
        await ctx.send('🔂 Current music is being looped.')
    ...
    async def _play(self, ctx: commands.Context, *, search: str):
      async with ctx.typing():
                try:
                    ...
                    await ctx.send('▶ Now enqueued {}'.format(str(source)))
    @_join.before_invoke
    @_play.before_invoke
    async def ensure_voice_state(self, ctx: commands.Context):
        if not ctx.author.voice or not ctx.author.voice.channel:
            raise commands.CommandError('⚠ You are not connected to any voice channel. Select any available voice channel first then summon me.')

        if ctx.voice_client:
            if ctx.voice_client.channel != ctx.author.voice.channel:
                raise commands.CommandError('⚠ I\'m already in the voice channel. Waiting for your commands.')
